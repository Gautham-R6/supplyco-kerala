import { ChevronLeft, ShoppingCart, Plus, Sparkles, PackageX } from "lucide-react";
...
export default function SabariBrands({
  ...
  // FAIL-SAFE CHECK: If there are no Sabari products yet, return a clean notice instead of crashing!
  if (sabariProducts.length === 0) {
    return (
      <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center bg-white rounded-2xl border border-[#eae2de] shadow-sm">
          <PackageX className="w-12 h-12 text-slate-300 mb-3" />
          <h3 className="font-bold text-slate-700 text-sm">
            {t("No Sabari Products Available", "ശബരി ഉൽപ്പന്നങ്ങൾ ലഭ്യമല്ല", user.language)}
          </h3>
          <p className="text-xs text-slate-400 mt-1 max-w-xs">
            {t("Please add Sabari inventory items inside your backend control layout dashboard.", "നിങ്ങളുടെ ബാക്കെൻഡ് കൺട്രോൾ ഡാഷ്‌ബോർഡിൽ ശബരി ഇനങ്ങൾ ചേർക്കുക.", user.language)}
          </p>
        </div>
      </div>
    );
  ...
  // Safe fallback definitions now that we know the list isn't empty
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col gap-2">
                <div className="space-y-0.5 text-[11px]">
                  ...
                  <div className="flex items-center gap-1 text-slate-650 leading-none">
                    ...
                    <span className={`font-semibold ${priceInfo.isOfferActive ? "line-through text-slate-400" : "text-[#00234e]"}`}>
                  ...
                  {priceInfo.soccerDiscountPct > 0 && (
                    <div className="text-[8px] font-bold bg-emerald-50 text-emerald-800 rounded px-1.5 py-0.5 border border-emerald-100 uppercase tracking-wide leading-none select-none block mt-1 w-fit">
                      Soccer Eleven: -{priceInfo.soccerDiscountPct}% Extra Off!
                    </div>
                  )}
                </div>
